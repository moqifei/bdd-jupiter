...
import static com.moqifei.bdd.jupiter.extension.ScenarioTestMethodContext.ResolverType.AGGREGATOR;
import static com.moqifei.bdd.jupiter.extension.ScenarioTestMethodContext.ResolverType.CONVERTER;
...
public class ScenarioTestMethodContext {

	private final List<ResolverType> resolverTypes;
	private final Resolver[] resolvers;

	ScenarioTestMethodContext(Method testMethod) {
	...
	/**
	 * Determine if the supplied {@link Parameter} is an aggregator (i.e., of type
	 ...
	 * <em>potentially</em> valid signature (i.e., formal parameter declarations)
	 * with regard to aggregators.
	 *
	 * <p>
	 * This method takes a best-effort approach at enforcing the following policy
	 * for parameterized test methods that accept aggregators as arguments.
	 *
	 * <ol>
	...
	/**
	 * Determine if the {@link Method} represented by this context declares at least
	 * one {@link Parameter} that is an {@linkplain #isAggregator aggregator}.
	 *
	 ...
	 * (i.e., of type {@link ArgumentsAccessor} or annotated with
	 * {@link AggregateWith}).
	 *
	 * @return {@code true} if the parameter is an aggregator
	 */
			...
			Resolver createResolver(ParameterContext parameterContext) {
				try { // @formatter:off
					return AnnotationUtils.findAnnotation(parameterContext.getParameter(), ConvertWith.class)
							...
							.map(converter -> AnnotationConsumerInitializer.initialize(parameterContext.getParameter(),
									converter))
